(() => {
	...
	const TC_MARKER = 'TokenCave';

	// Capture original fetch before anyone else can wrap it
	const originalFetch = window.fetch;

	// Wrap history methods early to detect SPA navigation
	const originalPushState = history.pushState.bind(history);
	const originalReplaceState = history.replaceState.bind(history);

	history.pushState = function (...args) {
		const result = originalPushState(...args);
		window.dispatchEvent(new CustomEvent('tc:urlchange'));
		return result;
	};
	...
	function post(type, payload) {
		window.postMessage({ tc: TC_MARKER, type, payload }, window.location.origin);
	}

	function postResponse(requestId, ok, payload, error) {
		window.postMessage({ tc: TC_MARKER, type: 'tc:response', requestId, ok, payload, error }, window.location.origin);
	}
	...
	async function handleConversationResponse({ orgId, conversationId }, response) {
		try {
			...
			post('tc:conversation', { orgId, conversationId, data });
		} catch {
				...
				for (const line of lines) {
					...
					try {
						...
						if (json?.type === 'message_limit' && json.message_limit) {
							post('tc:message_limit', json.message_limit);
						}
					} catch {
						// ignore
					}
	...
	window.addEventListener('message', async (event) => {
		...
		if (!data || data.tc !== TC_MARKER) return;
		if (data.type !== 'tc:request') return;
		...
		try {
			...
			if (kind === 'conversation') {
				...
				const json = await res.json();
				post('tc:conversation', { orgId, conversationId, data: json });
				postResponse(requestId, true, json, null);
				return;
			}
