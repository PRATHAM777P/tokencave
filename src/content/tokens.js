(() => {
	...
	async function hashString(str) {
		if (!TC.bridge?.requestHash) return null;
		try {
			const res = await TC.bridge.requestHash(str);
	...
	async function computeConversationMetrics(conversation) {
		...
		const cachedUntil = lastAssistantMs ? lastAssistantMs + TC.CONST.CACHE_WINDOW_MS : null;

		return { trunkMessageCount: trunk.length, totalTokens, lastAssistantMs, cachedUntil };
	}

	TC.tokens = { computeConversationMetrics, countTokens };
})();
