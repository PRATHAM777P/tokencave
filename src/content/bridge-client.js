(() => {
	...
	class BridgeClient {
		constructor() {
			...
			window.addEventListener('message', (event) => {
				...
				if (!data || data.tc !== 'TokenCave') return;

				if (data.type === 'tc:response') {
	...
	function injectBridgeOnce() {
		...
		if (document.getElementById(TC.DOM.BRIDGE_SCRIPT_ID)) {
			return Promise.resolve(true);
		}

		bridgeReadyPromise = new Promise((resolve) => {
			const script = document.createElement('script');
			script.id = TC.DOM.BRIDGE_SCRIPT_ID;
	...
	TC.bridge = new BridgeClient();
	TC.injectBridgeOnce = injectBridgeOnce;
})();
